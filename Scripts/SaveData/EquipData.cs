...
namespace GGemCo.Scripts.SaveData
{
    ...
    /// 인벤토리 아이템 관리
    /// </summary>
    public class EquipData : DefaultData, ISaveData
    {
        ...
        {
            if (saveDataContainer?.EquipData == null) return;
            ItemCounts.Clear();
            foreach (var info in saveDataContainer.EquipData.ItemCounts)
            {
        ...
        public ResultCommon AddItem(int itemUid, int value)
        {
            if (maxCountIcon <= 0)
            {
                UIWindowEquip uiWindowEquip =
                    SceneGame.Instance.uIWindowManager.GetUIWindowByUid<UIWindowEquip>(UIWindowManager.WindowUid
                        .Equip);
                if (uiWindowEquip != null)
                {
                    maxCountIcon = uiWindowEquip.maxCountIcon;
                }
            }
